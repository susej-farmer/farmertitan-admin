...
class EquipmentCatalogService {
  static async findAll(options = {}) {
    try {
      ...
      console.error('Error in EquipmentCatalogService.findAll:', error);
      throw error;
    }
  }
  ...
  static async findById(id) {
    try {
      // Validate ID
      if (!id) {
        throw new AppError('Equipment catalog ID is required', 400, 'MISSING_ID');
      }
      ...
      console.error('Error in EquipmentCatalogService.findById:', error);
      throw error;
    }
  }
  ...
  static async findTrimsForModel(makeId, modelId) {
    try {
      ...
      console.error('Error in EquipmentCatalogService.findTrimsForModel:', error);
      throw error;
    }
  }
  ...
  static async create(equipmentData) {
    try {
      ...
      console.error('Error in EquipmentCatalogService.create:', error);
      throw error;
    }
  }
  ...
  static async update(id, updateData) {
    try {
      ...
      console.error('Error in EquipmentCatalogService.update:', error);
      throw error;
    }
  }
  ...
  static async delete(id) {
    try {
      ...
      console.error('Error in EquipmentCatalogService.delete:', error);
      throw error;
    }
  }
  ...
  static async findByTypeModelTrim(type, make, model, trim = null, year = null) {
    try {
      ...
      console.error('Error in EquipmentCatalogService.findByTypeModelTrim:', error);
      throw error;
    }
  }

  /**
   * Buscar _equipment por atributos (campos opcionales excepto type y make)
  ...
  static async findByAttributes(attributes) {
    try {
      ...
      if (!attributes || !attributes.type || !attributes.make) {
        throw new AppError('Type and make are required', 400, 'MISSING_REQUIRED_PARAMS');
      }

      return await EquipmentCatalogClient.findByAttributes(attributes);
    } catch (error) {
      console.error('Error in EquipmentCatalogService.findByAttributes:', error);
      throw error;
    }
  }
...
module.exports = EquipmentCatalogService;
