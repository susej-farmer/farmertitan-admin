...
class PartTypeClient {
  ...
  static async findAll(options = {}) {
    try {
      ...
      // For each part type, get user and farm names
      ...
      console.error('Failed to find all part types', error, { options });
      throw error;
    }
  }
    ...
    try {
      ...
      if (error) {
        if (error.code === 'PGRST116') {
          throw new AppError('Part type not found', 404, 'PART_TYPE_NOT_FOUND');
        }
        throw error;
      }
      ...
      console.error('Failed to find part type by ID', error, { id });
      throw error;
    }
  }
  ...
  static async findForDropdown() {
    try {
      ...
      console.error('Failed to find part types for dropdown', error);
      throw error;
    }
  }

  static async create(partTypeData) {
    try {
      const { name, description = null, created_by = null, created_in = null } = partTypeData;
      ...
      console.log('Part type created successfully', { id: data.id, name: data.name });
      
      return data;
    } catch (error) {
      console.error('Failed to create part type', error, { partTypeData });
      throw error;
    }
  }
  ...
  static async update(id, updateData) {
    try {
      ...
      console.log('Part type updated successfully', { id, fields: Object.keys(updateObject) });
      
      return data;
    } catch (error) {
      if (error.isOperational) throw error;
      console.error('Failed to update part type', error, { id, updateData });
      throw error;
    }
  }
  ...
  static async delete(id) {
    try {
      ...
      if ((taskCount || 0) > 0) {
        throw new AppError(
          'Cannot delete part type with associated tasks',
          409,
          'PART_TYPE_HAS_DEPENDENCIES',
          {
      ...
      console.log('Part type deleted successfully', { id });
      
      return data;
    } catch (error) {
      if (error.isOperational) throw error;
      console.error('Failed to delete part type', error, { id });
      throw error;
    }
  }
  ...
  static async checkNameUnique(name, excludeId = null) {
    try {
      ...
      console.error('Error checking part type name uniqueness:', error);
      return false;
    }
  }
}

module.exports = PartTypeClient;