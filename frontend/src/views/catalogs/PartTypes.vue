<template>
  <div class="space-y-6">
    ...
    <div v-if="loading" class="text-center py-8">
      ...
      <p class="text-gray-600">Loading part types...</p>
    </div>
    ...
    <div v-else-if="error" class="card">
      <div class="card-body text-center py-8">
        ...
        <button @click="loadPartTypes" class="btn btn-primary">
          Try Again
        </button>
      </div>
    </div>
        ...
        <table class="table min-w-full">
          ...
          <tbody class="table-body">
            <tr v-for="partType in partTypes" :key="partType.id" class="table-row">
              ...
              <td class="table-cell">{{ partType.created_in_name || '-' }}</td>
              <td class="table-cell">{{ formatDate(partType.created_at) }}</td>
              <td class="table-cell">
                <div class="flex items-center space-x-2">
                  <button 
                    @click="editPartType(partType)"
    ...
    <div v-if="showCreateModal || showEditModal" class="fixed inset-0 z-50 overflow-y-auto">
      <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          <form @submit.prevent="savePartType">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">
                {{ showEditModal ? 'Edit Part Type' : 'Create Part Type' }}
              </h3>
              ...
              <div class="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    class="form-input"
                    rows="3"
                    placeholder="Enter part type description (optional)"
                  ></textarea>
                </div>
              </div>
            </div>
...
export default {
  ...
  setup() {
    ...
    const loadPartTypes = async () => {
      ...
      try {
        ...
        if (response.success) {
          ...
          throw new Error(response.message || 'Failed to load part types')
        }
      } catch (err) {
        console.error('Failed to load part types:', err)
        error.value = err.userMessage || err.message || 'Failed to load part types'
        partTypes.value = []
      } finally {
        loading.value = false
      }
    ...
    const editPartType = (partType) => {
      ...
      form.description = partType.description || ''
      showEditModal.value = true
    }
    
    const deletePartType = async (partType) => {
      const confirmed = await confirmDelete(partType.name)
      if (!confirmed) return
      
      try {
        const response = await catalogsApi.deletePartType(partType.id)
        if (response.success) {
          success('Part type deleted successfully')
          loadPartTypes()
        } else {
          throw new Error(response.message || 'Failed to delete part type')
        }
      } catch (err) {
    ...
    const savePartType = async () => {
      ...
      try {
        ...
        if (showEditModal.value) {
          response = await catalogsApi.updatePartType(form.id, data)
        } else {
          response = await catalogsApi.createPartType(data)
        }
        
        if (response.success) {
          success(`Part type ${showEditModal.value ? 'updated' : 'created'} successfully`)
          closeModal()
          loadPartTypes()
        } else {
          throw new Error(response.message || 'Failed to save part type')
        }
      } catch (err) {
